...
- 未設定なら **`127.0.0.1:8001`**（localhost のみ）。
- 全インターフェースで待ち受けるなら `PI_INFO_HOST=0.0.0.0`。特定のIPに絞るなら
  そのIPを指定。
- 別コンテナのリバースプロキシを前段に置く場合は、そのプロキシから到達できる
  アドレス（docker ネットワークのゲートウェイIP等）を `PI_INFO_HOST` に指定します。
  ホスト側のファイアウォールでコンテナ→ホストの通信を遮断していると届かないため、
  必要に応じて該当サブネットから当該ポートへの通信を許可してください。
- `network_mode: host` のため `ports:` 指定は使いません。ポートは `PI_INFO_PORT` で変更します。
- フロントの API 取得は表示中ページからの相対パスなので、サブパス配下にマウント
  しても動作します（末尾スラッシュ付きの URL で開く）。
